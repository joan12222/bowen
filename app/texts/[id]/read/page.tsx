...
export default function ReadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/annotations?textId=${id}`).then((r) => r.json()),
    ])
      .then(([t, a]) => {
        setText(t)
        setAnnotations(a)
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="text-center py-12 text-gray-400">
          ...
          <Link href={`/texts/${id}/import`} className="text-indigo-600 text-sm">
            前往导入 Word 文档
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        <Link href={`/texts/${id}`} className="text-gray-500 flex-shrink-0">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-5 h-5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
