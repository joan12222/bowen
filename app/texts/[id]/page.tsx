...
export default function TextDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  ...
  async function saveContent() {
    ...
    try {
      await fetch(`/api/texts/${id}/content`, {
        method: "PATCH",
  ...
  useEffect(() => {
    function loadTab<T>(url: string, cacheKey: string, setter: (v: T) => void) {
      const cached = localStorage.getItem(cacheKey)
    ...
    if (tab === "annotations") {
      loadTab(`/api/annotations?textId=${id}`, `ann_${id}`, setAnnotations)
    } else if (tab === "sentences") {
      loadTab(`/api/sentences?textId=${id}`, `sent_${id}`, setSentences)
    } else if (tab === "recitations") {
      loadTab(`/api/recitations?textId=${id}`, `rec_${id}`, setRecitations)
    }
  }, [tab, id])
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {text.content ? (
        <div className="mb-4 flex gap-2">
          <Link href={`/texts/${id}/read`} className="flex-1">
          ...
          <button
            onClick={async () => {
              ...
              await fetch(`/api/texts/${id}/content`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ content: "" }) })
            ...
            <div className="flex justify-end mb-2">
              <button
                onClick={async () => {
                  ...
                  await fetch(`/api/texts/${id}/annotations`, { method: "DELETE" })
                  setAnnotations([])
            ...
            <div className="flex justify-end mb-2">
              <button
                onClick={async () => {
                  ...
                  await fetch(`/api/texts/${id}/sentences`, { method: "DELETE" })
                  setSentences([])
            ...
            {recitations.length > 0 && (
              <button
                onClick={async () => {
                  ...
                  await fetch(`/api/texts/${id}/recitations`, { method: "DELETE" })
                  setRecitations([])
